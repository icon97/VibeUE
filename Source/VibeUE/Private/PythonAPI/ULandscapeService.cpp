	...
	if (Info)
	{
		for (const FLandscapeInfoLayerSettings& LayerSettings : Info->Layers)
		{
			...
			if (LayerSettings.LayerInfoObj)
			{
				...
				LayerInfo.bIsWeightBlended = LayerSettings.LayerInfoObj->bNoWeightBlend == 0;
			}
			else
			{
	...
	float WorldX, float WorldY)
{
	...
	for (const FLandscapeInfoLayerSettings& LayerSettings : Info->Layers)
	{
		...
		FAlphamapAccessor<false, false> AlphaAccessor(Info, LayerSettings.LayerInfoObj);
...
#else
		LandscapeInfo->ApplySplines(false, nullptr, true);
#endif
